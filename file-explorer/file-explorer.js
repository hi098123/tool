...
(function() {
	...
	window.FileExplorer = function(parentelem, options) {
		...
		var elems = {
			...
			navtool_back: CreateNode('button', ['fe_fileexplorer_navtool_back', 'fe_fileexplorer_disabled'], { title: $this.Translate('뒤로 (Alt + 왼쪽 화살표)'), tabIndex: -1 }),
			navtool_forward: CreateNode('button', ['fe_fileexplorer_navtool_forward', 'fe_fileexplorer_disabled'], { title: $this.Translate('앞으로 (Alt + 오른쪽 화살표)'), tabIndex: -1 }),
			navtool_history: CreateNode('button', ['fe_fileexplorer_navtool_history'], { title: $this.Translate('최근 경로') }),
			navtool_up: CreateNode('button', ['fe_fileexplorer_navtool_up', 'fe_fileexplorer_disabled'], { title: $this.Translate('위로 (Alt + 위쪽 화살표)'), tabIndex: -1 }),
		...
		var UpdateSelectionsChanged = function() {
			...
			else if (numselecteditems > 1)
			{
				...
				else  $this.SetNamedStatusBarText('selected', EscapeHTML(FormatStr($this.Translate('{0}개 항목 선택함'), numselecteditems)));
			}
			else
			{
				if (numfound)  $this.SetNamedStatusBarText('selected', EscapeHTML(FormatStr($this.Translate('1 item selected {0}'), sizestr)));
				else  $this.SetNamedStatusBarText('selected', EscapeHTML($this.Translate('1개 항목 선택함')));
			}
		...
		var FinalizeSyncCurrFolderToDOM = function(historyselected, selchanged) {
			...
			else
			{
				...
				else  elems.itemsmessagewrap.innerHTML = EscapeHTML($this.Translate('이 폴더는 비어 있습니다.'));
			...
			else  $this.SetNamedStatusBarText('folder', EscapeHTML(FormatStr($this.Translate('{0}개 항목'), entries.length)));
		...
		var NavigationChanged = function() {
			...
			if (currhistory <= 0)
			{
				...
				elems.navtool_back.title = $this.Translate('뒤로 (Alt + Left Arrow)');
			}
			else
			{
			...
			if (currhistory >= historystack.length - 1)
			{
				...
				elems.navtool_forward.title = $this.Translate('앞으로 (Alt + 오른쪽 화살표)');
			}
			else
			{
			...
			if (currpath.length <= 1)
			{
				...
				elems.navtool_up.title = $this.Translate('위로 (Alt + 위쪽 화살표)');
			}
			else
			{
...
*/
	...
	var FileExplorerTool_Upload = function(fe) {
		...
		var node = fe.AddToolbarButton('fe_fileexplorer_folder_tool_upload', fe.Translate('불러오기 (Ctrl + U)'));
	...
	var FileExplorerTool_Download = function(fe) {
		...
		var node = fe.AddToolbarButton('fe_fileexplorer_folder_tool_download', fe.Translate('저장하기'));
		...
		var ClickHandler = function(e) {
			...
			fe.SetNamedStatusBarText('message', EscapeHTML(fe.Translate('다운로드 준비중....')), fe.settings.messagetimeout);
	...
	var FileExplorerTool_Delete = function(fe) {
		...
		var node = fe.AddToolbarButton('fe_fileexplorer_folder_tool_delete', fe.Translate('지우기 (Del)'));
